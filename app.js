(function(){
  ...
  async function requestGemini(text){
    ...
    const res=await fetch(url,{
      ...
      body:JSON.stringify({
        systemInstruction:{parts:[{text:state.sysPrompt || 'You are a concise, helpful voice assistant.'}]},
        contents:[{role:'user',parts:[{text}]}]
      })
    });
    if(!res.ok) await parseFailure(res,'Gemini');
    const data=await res.json();
    ...
    if(block) throw new Error('Gemini returned no text ('+block+').');
    return '';
  }
